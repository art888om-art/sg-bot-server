...
REPORTS_PAGE = f"""<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Отчёты</title>{_BASE_CSS}</head><body>{_NAV}<div class="container"><h1>📈 Отчёты</h1><div class="stats-grid" id="report_cards"></div><div class="card mt-4"><canvas id="chartFunnel" style="max-height:300px"></canvas></div></div><script>
async function load(){{const r=await fetch('/api/analytics');if(r.status===403){{window.location.href='/';return}}const a=await r.json();document.getElementById('report_cards').innerHTML=`<div class="card"><div class="card-header">Выручка всего</div><div class="card-value">${{a.total_revenue.toLocaleString('uk-UA')}} ₴</div></div><div class="card"><div class="card-header">Сделок всего</div><div class="card-value">${{a.total_deals}}</div></div><div class="card"><div class="card-header">Конверсия</div><div class="card-value">${{a.conversion}}%</div></div><div class="card"><div class="card-header">Выручка за месяц</div><div class="card-value">${{a.month_revenue.toLocaleString('uk-UA')}} ₴</div></div>`;new Chart(document.getElementById('chartFunnel'),{{type:'bar',data:{{labels:['Сделки','Выручка'],datasets:[{{label:'Всего',data:[a.total_deals,a.total_revenue],backgroundColor:['#f5a623','#22c55e']}}]}},options:{{responsive:true,plugins:{{legend:{{display:false}}}}}}}});}}
load();
</script></body></html>"""

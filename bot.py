...
LOGIN_PAGE = f"""<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AutoCRM — Вход</title>{_BASE_CSS}</head><body><div class="login-wrap"><div class="login-card"><div class="login-logo">⚡</div><h1>AutoCRM</h1><p>CRM для продажи генераторов и стартеров</p><input type="text" id="tg_id" placeholder="Ваш Telegram ID"><input type="text" id="name_inp" placeholder="Ваше имя (первый вход)"><button class="btn btn-primary" onclick="doLogin()">Войти</button><div id="err" style="color:var(--red);font-size:13px;margin-top:12px"></div></div></div><script>
